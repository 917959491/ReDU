...
def write_XYSIZE_data(graphs,labels,sizes,NUM_SEQUENCE,index,max_num, filename):
    ...
    for key,graph in graphs.items():
        ...
        id_data.append(id)
        x_data.append(temp)
        all_label.append(y+size_temp)
        y_data.append(np.log(y+1.0)/np.log(2.0))
        # y_data.append(np.log(size_temp + 1.0) / np.log(2.0))
        # y_data.append(y)
        Laplacian_data.append(Laplacian)
        sz_data.append(size_temp)
    gc.collect()
    pickle.dump((id_data,x_data,Laplacian_data,y_data, sz_data, time_data,index.length(),all_label), open(filename,'wb'))

...
n_steps = 100 #ren:100 weibo:53 twitter:23
...
tf.flags.DEFINE_integer("num_kernel", 3, "chebyshev .") #ren:3 weibo:2
tf.flags.DEFINE_float("learning_rate", 0.005, "learning_rate.")  ##0.005
tf.flags.DEFINE_integer("batch_size", 16, "batch size.") #32
...
tf.flags.DEFINE_integer("training_iters", 16*1600 + 1, "max training iters.") #200 * 3200 +1
...
tf.flags.DEFINE_integer("feat_in", 100, "num of feature in") #ren:100 weibo:53 twitter:23
tf.flags.DEFINE_integer("feat_out", 50, "num of feature out")
tf.flags.DEFINE_integer("lmax", 2, "max L")
tf.flags.DEFINE_integer("num_nodes", 100, "number of max nodes in cascade") #ren:100 weibo:53 twitter:23
...
id_train, x_train, L, y_train, sz_train, time_train, vocabulary_size,_ = pickle.load(
    open(cf.train_pkl, 'rb'))
id_test, x_test, L_test, y_test, sz_test, time_test, _,_ = pickle.load(
    open(cf.test_pkl, 'rb'))
id_val, x_val, L_val, y_val, sz_val, time_val, _,_ = pickle.load(open(cf.val_pkl, 'rb'))
...
while step * batch_size < training_iters:
    ...
    if step % display_step == 0:
        ...
        pickle.dump((predict_result, y_test, test_loss), open(
            "prediction_result__renminwang" + str(config.learning_rate) +str(step)+ "_CasCN", 'wb'))
        ...
        ##11.30
        save_path = saver.save(sess, "save/model.ckpt", global_step=step)
...
##11.30
save_path = saver.save(sess, "save/final_model.ckpt",global_step=step)
print("Model saved in file: %s" % save_path)